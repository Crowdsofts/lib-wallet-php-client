...
class HeaderBag
{
    ...
    public function setHeaders(array $headers)
    {
        ...
        foreach ($headers as $headerName => $headerValue) {
            if (is_numeric($headerName) && is_string($headerValue) && str_contains($headerValue, ': ')) {
                [$headerName, $headerValue] = explode(': ', $headerValue, 2);
            }
    ...
    protected function normalizeHeaderName($header)
    {
        ...
        return preg_replace_callback('/\-(.)/', [$this, 'headerNormalizerCallback'], ucfirst($header));
    }
